...
## 6. 策略变更的代价与产物版本化（2026-08-29 讨论记录）

改变预处理策略的成本按"改什么"分三档：

| 档 | 改动内容 | 改动位置 | 代价 |
|---|---|---|---|
| **调参数**（80% 场景） | 时间网格 interval / 起始时间 / 通道范围 / n_context / delays / GPT-2 层 / eliminate_data | `configs/preprocess.yaml` 改值 | 改配置 + 重跑（sidecar 参数不匹配自动失效旧产物，重跑自动再生） |
| **换流程**（15%） | 换语义编码器、加处理步骤、新模态 adapter | 一个函数 + config 开关；训练侧改 `data.layer` 等 | 小改动；数据形状不变则训练代码零改动 |
| **换范式**（5%） | 换对齐策略（trial-based）、换任务定义 | 算法本身 | 大改动——这在任何框架都一样，非工程成本 |

**已知弱点（当前未解决，YAGNI 决策）**：产物文件名没有参数版本化——
不同参数生成的产物同名（如 `zresp1_1.npy`），改参数重跑会**覆盖**旧产物，
新旧策略无法并存对比。临时方案 = 换 `processed_root` 目录。
计划方案 = CLI 加 `--tag`（产物写 `processed_root/{tag}/`，训练侧
`discover` 选 tag，约 30-50 行）——**等第一次真正要变策略时再做**，
届时再决定 tag 应记录什么。
